...
import Exceptions.DeserializableClassNotEquals;
import Exceptions.DirectoryCreationException;
import Exceptions.ObjectNotInstanceOfSerializable;
...
public class FileService {
    ...
    public static MarketService deserializeMarket(String filePath, String fileName) {
        ...
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream(fullFilePath))) {
            ...
            if (!(obj instanceof Serializable))
            {
                throw new ObjectNotInstanceOfSerializable("Object not instance");
            }
            if(MarketService.class.equals(obj.getClass())) {
                System.out.println("Deserialization completed successfully.");
                return  ((MarketService) obj);
            } else {
                throw new DeserializableClassNotEquals("Class not equals");
            }
        } catch (IOException | ClassNotFoundException | DeserializableClassNotEquals | ObjectNotInstanceOfSerializable e) {
            System.out.println("Error during deserialization: " + e.getMessage());
            return null;
        }
    }
